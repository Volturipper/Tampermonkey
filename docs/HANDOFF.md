...
- GitHub raw finding: `Volturipper/chatgpt-auto-continue-review` is private, so unauthenticated Tampermonkey raw update URLs there return 404. Added first real public workspace script `scripts/cac-v220-runtime/cac-v220-runtime.user.js` with `@updateURL` / `@downloadURL` pointing to public `Volturipper/Tampermonkey`. Metadata version is `2026.5.220` because Tampermonkey treated lower numeric variants as a downgrade from the package-installed date version; runtime `VERSION` remains `v220-lease-alias-candidate.1`. `npm run tm:check`, `npm test`, and `node --check` passed before commit.
- 2026-05-20: restored the dedicated Chrome Dev Tampermonkey profile after the public-update attempt left all CAC rows disabled. Reinstalled the public workspace script `scripts/cac-v220-runtime/cac-v220-runtime.user.js` through `CAC_TAMPERMONKEY_WHITELIST_INSTALL` with explicit runtime URL/conversation id, then verified exactly one enabled CAC row: `ChatGPT Auto Continue - CAC API Enhancement Candidate v220` version `2026.5.220`. Latest scoped verify: `target_has_api=true`, `other_pages_with_api=0`, script SHA256 `60262a25798b8da77ca3dce7e9634efbe5ddfe9c87f8dafab7385e646ccce4c7`.
- Public v220 restore checks passed after reinstall: `CAC_API_SMOKE` key count 27, `CAC_SUPERVISED_CONTROL_SMOKE` `dry_run_continue_ok=true` and `zero_real_actions=true`, `CAC_LEASE_GATE` `LEASE_DRY_RUN_READY_REAL_BLOCKED`, default `CAC_RELEASE_GATE` `SUPERVISED_DRY_RUN_READY_REAL_BLOCKED`, and `CAC_API_HEARTBEAT_INGEST` refreshed `CAC_MONITOR` to `decision=ACTIVE`.
- Added reusable public raw preflight: `npm run tm:raw-check`. It fetches each userscript's public `@updateURL` / `@downloadURL`, verifies remote `@version` matches local metadata, and prints fetched SHA256. Latest real check passed for CAC v220 with SHA256 `60262a25798b8da77ca3dce7e9634efbe5ddfe9c87f8dafab7385e646ccce4c7`.
- Raw auto-update test plan is recorded in `docs/START_HERE.md`: use a metadata-only version bump on the same public script URL, update the existing Tampermonkey row only, then rerun scoped verify and CAC smoke gates. Never leave two enabled CAC rows.
- 2026-05-20: executed the first GitHub raw auto-update test. Metadata-only bump `2026.5.220 -> 2026.5.221` was committed and pushed to public `Volturipper/Tampermonkey`; runtime `VERSION` stayed `v220-lease-alias-candidate.1`. `npm run tm:raw-check` returned CAC SHA256 `9a06ae1907f1309969f559a142ee4c5d2f1bcefc088320dfcd1e643d9595997f`.
- Tampermonkey update result: clicked the existing enabled v220 row's `Check for updates` control in the Chrome Dev profile; the row updated to metadata version `2026.5.221` with exactly one enabled CAC row. Scoped verify stayed `target_has_api=true`, `other_pages_with_api=0`. Post-update `CAC_API_SMOKE`, `CAC_SUPERVISED_CONTROL_SMOKE`, `CAC_LEASE_GATE`, `CAC_RELEASE_GATE`, and `CAC_API_HEARTBEAT_INGEST` passed; release gate still blocks real/unattended/production/takeover.
- Next CAC API enhancement selected: `v221-self-iteration-packet`, recorded in `prompts/cac-v221-self-iteration-packet-request.md`. It asks for a read-only `selfIterationPacket()` API plus current contract-summary alignment because `CAC_API_CONTRACT_SUMMARY` still reports an older v216/18-key surface while the live v220 runtime has 27 keys.
- Sent `prompts/cac-v221-self-iteration-packet-request.md` to the CAC maintainer page with `WEBAI_CHATGPT_DIRECT_SEND` using the dedicated Chrome Dev profile. Direct-send latest receipt: `D:\Codex\n8n-gpt-orchestrator\scratch\webai-chatgpt-direct-send-latest.json`. Role heartbeat `cac-maintainer-agent-api` was recorded as `task_sent` / `waiting_for_v221_self_iteration_packet` in `D:\Codex\n8n-gpt-orchestrator\scratch\cac-agent-api-gate-webai-heartbeat.json`.
