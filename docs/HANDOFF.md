...
- GitHub raw finding: `Volturipper/chatgpt-auto-continue-review` is private, so unauthenticated Tampermonkey raw update URLs there return 404. Added first real public workspace script `scripts/cac-v220-runtime/cac-v220-runtime.user.js` with `@updateURL` / `@downloadURL` pointing to public `Volturipper/Tampermonkey`. Metadata version is `220.0.1` to satisfy project semver checks; runtime `VERSION` remains `v220-lease-alias-candidate.1`. `npm run tm:check`, `npm test`, and `node --check` passed before commit.
